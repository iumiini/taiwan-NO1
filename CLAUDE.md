...
**新 session 先讀《D1｜開發接力筆記》**：開機流程、協作規則、權威索引、現在在做什麼。

| 用途 | 頁面 |
|---|---|
| 開機與接力（地圖） | [D1｜開發接力筆記](https://app.notion.com/p/3e4fd52e676481c6baa6c6d756e205ae) |
| 總綱權威（衝突以此為準） | [00｜藍圖](https://app.notion.com/p/3e4fd52e676481e0a005f8b877fc2d26) |
| 規格現況（給人看） | Inbox 編號頁 `01`～`05`、[T1｜資料管線](https://app.notion.com/p/3e4fd52e6764816f8cddd99908ede2c8) |
| 待決策 | [09｜待決策清單](https://app.notion.com/p/3e4fd52e6764814aaa77c9c6549e3c1e) |
| 決策歷史（考古用） | [H｜設計討論紀錄](https://app.notion.com/p/396fd52e67648143845ed7a667dd85ba)（原《設計討論總結》，不再新增章節） |
...
專案頁的摺疊區塊**已凍結**。Inbox 頁用「編號｜」前綴（全形直線）：`00`～`09` 設計、`D` 開發協作、`T` 技術、`H` 歷史（仍被引用、只增不改）、`Z` 封存；各頁清單以
專案頁的 Inbox relation 動態讀取為準，不要依賴本表。

**權威頁的寫法**：只寫規則與意圖，數值指向 `balance.json` 不抄值；每條規則註明
對應的 `spec:id`；修改時不刪、只劃線，頁尾〈🕓 變更紀錄〉加一列。
...
4. 若答案牽涉設計規格的**新增或修正**，走 `/設計變更`（見下節）：同步改程式／
   `spec.json`／對應的 Notion 權威頁
5. **自己提出的待決策問題也要寫進 Notion**，不能只留在對話裡。只出現在對話中的
   問題，使用者關掉視窗就找不到了。寫進《09｜待決策清單》
...
| 層 | 檔案 | 性質 |
|---|---|---|
| 規格現況（給人看） | Notion 權威頁 `00`～`05`、`T1` | 規則、意圖、狀態；不刪只劃線 |
| 規格現況（給程式） | `data/spec.json` | **每項只出現一次**，會被覆寫；以 `id` 與權威頁對應 |
| 數值 | `data/balance.json` | 程式與文件共用的單一來源 |
| 決策理由與歷史 | Notion `H｜` 與各張協作頁 | 只增不改的航海日誌 |

`docs/spec-current.md` 由 `tools/gen_spec.py` 從 spec.json 與 balance.json 生成，
**不要手動編輯**。數值以路徑指向 balance.json，生成時代入。

### /設計變更（動到規格就走這五步）

1. **查碼**：講出實際算式與數值
2. **比規格**：對照 Notion 權威頁，列出變更前／變更後
3. **等使用者說「執行」**
4. **實作＋同步三處**：程式／`balance.json`、`data/spec.json`（重跑生成器）、
   Notion 權威頁；再回填協作頁的 A
5. **不刪、只劃線**：權威頁舊內容加刪除線並寫正確內容，頁尾變更紀錄加一列

自己提出的待決策問題：`spec.json` 填 `status: undecided`，並寫進《09｜待決策清單》。
